...
const Navbar = () => {
  ...
  const statistics = location.pathname === "/statistics";
      ...
      >
        <div onClick={changeNav} className="md:hidden absolute right-4 z-40">
          {nav ? (
            ...
            <IoMenu size={25} color={homepage ? "white" : "black"} />
          )}
        </div>
        <div
          className={`flex ease-in-out duration-300 h-3/4 shadow-2xl items-center justify-center drop-shadow-xl ${
            nav
        ...
        >
          <div className="w-2/3 h-2/3 flex py-6 items-center justify-center flex-col">
            ...
            {!loggedIn ? (
              ...
              <button
                onClick={logout}
                className={`text-gray-950 font-noto-sans font-bold text-3xl transition-all duration-300 hover:scale-125 hover:text-golf my-12`}
              >
                Logout
              </button>
            )}
        ...
        <div className="flex flex-row items-center md:visible invisible z-50">
          ...
          <Link
            to="/courses"
            className={`${
              !homepage ? "text-gray-900" : "text-white"
